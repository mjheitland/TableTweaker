...
namespace TableTweaker.Roslyn
{
    public class InteractiveManager
    {
        ...
        public InteractiveManager()
        {
            ...
            _references = AssemblyTypes.Select(t =>
            ...
            _compilationOptions = new CSharpCompilationOptions(OutputKind.DynamicallyLinkedLibrary,
                usings: AssemblyTypes.Select(x => x.Namespace).ToImmutableArray());
            ...
            if (GetCurrentDocument().TryGetText(out text))
            {
                CSharpScript.Run(text.ToString(),
                    ScriptOptions.Default
                        .AddNamespaces(AssemblyTypes.Select(x => x.Namespace))
                        .AddReferences(AssemblyTypes.Select(x => x.Assembly)));
            }
        }
    }
}
